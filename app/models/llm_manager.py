"""Singleton GLM text model loader and inference gateway."""

from __future__ import annotations

import os
import threading
from dataclasses import dataclass
from time import perf_counter
from typing import Any, Final

import torch
from transformers import AutoModelForCausalLM, AutoTokenizer

from app.config.llm_settings import LlmSettings
from app.core.exceptions import ModelException
from app.utils.logging import get_logger

logger = get_logger(__name__)

_SYSTEM_PROMPT: Final[str] = (
    "You are a KYC document extraction engine. "
    "Return only valid JSON matching the requested schema. "
    "Use null for unknown fields. Do not invent data."
)


@dataclass(frozen=True, slots=True)
class LlmArtifacts:
    """Loaded LLM artifacts."""

    tokenizer: Any
    model: Any
    device: torch.device
    dtype: torch.dtype


class LlmManager:
    """Process-wide singleton for GLM text generation."""

    _instance: LlmManager | None = None
    _lock: threading.Lock = threading.Lock()

    def __init__(self, settings: LlmSettings) -> None:
        self._settings = settings
        self._artifacts: LlmArtifacts | None = None

    @classmethod
    def get_instance(cls, settings: LlmSettings) -> LlmManager:
        if cls._instance is not None:
            return cls._instance
        with cls._lock:
            if cls._instance is None:
                cls._instance = cls(settings)
        return cls._instance

    @property
    def is_loaded(self) -> bool:
        return self._artifacts is not None

    def load(self) -> None:
        if self._artifacts is not None:
            return

        start = perf_counter()
        logger.info("LLM loading started (model_id=%s)", self._settings.model_id)

        try:
            self._apply_hf_cache_env()
            dtype = _parse_torch_dtype(self._settings.torch_dtype)
            device = _resolve_device(self._settings.device)

            tokenizer = AutoTokenizer.from_pretrained(
                self._settings.model_id,
                trust_remote_code=True,
            )
            model = AutoModelForCausalLM.from_pretrained(
                self._settings.model_id,
                torch_dtype=dtype,
                trust_remote_code=True,
                device_map=None,
            )
            model.eval()
            model.to(device)

            self._artifacts = LlmArtifacts(
                tokenizer=tokenizer,
                model=model,
                device=device,
                dtype=dtype,
            )
        except ModelException:
            logger.exception("LLM loading failed")
            raise
        except Exception as exc:
            logger.exception("LLM loading failed")
            raise ModelException("Failed to load LLM model.") from exc
        finally:
            if self._artifacts is not None:
                elapsed_ms = int((perf_counter() - start) * 1000)
                logger.info("LLM loaded (%d ms)", elapsed_ms)

    def generate_json(self, *, user_prompt: str) -> str:
        """Generate text completion for a chat prompt."""
        if self._artifacts is None:
            self.load()
        assert self._artifacts is not None

        tokenizer = self._artifacts.tokenizer
        model = self._artifacts.model

        messages = [
            {"role": "system", "content": _SYSTEM_PROMPT},
            {"role": "user", "content": user_prompt},
        ]

        try:
            model_inputs = tokenizer.apply_chat_template(
                messages,
                add_generation_prompt=True,
                return_tensors="pt",
            ).to(self._artifacts.device)

            with torch.inference_mode(), _autocast_if_available(
                self._artifacts.device, self._artifacts.dtype
            ):
                output_ids = model.generate(
                    model_inputs,
                    max_new_tokens=self._settings.max_new_tokens,
                    do_sample=self._settings.temperature > 0,
                    temperature=max(self._settings.temperature, 1e-5),
                )

            generated = output_ids[0][model_inputs.shape[-1] :]
            return tokenizer.decode(generated, skip_special_tokens=True).strip()
        except Exception as exc:
            logger.exception("LLM inference failed")
            raise ModelException("LLM inference failed.") from exc

    def _apply_hf_cache_env(self) -> None:
        os.environ.setdefault("HF_HOME", self._settings.hf_home)
        os.environ.setdefault("TRANSFORMERS_CACHE", self._settings.transformers_cache)


def _parse_torch_dtype(value: str) -> torch.dtype:
    normalized = value.strip().lower()
    if normalized == "float16":
        return torch.float16
    if normalized == "bfloat16":
        return torch.bfloat16
    if normalized == "float32":
        return torch.float32
    raise ValueError(f"Unsupported TORCH_DTYPE: {value!r}")


def _resolve_device(value: str) -> torch.device:
    normalized = value.strip().lower()
    if normalized.startswith("cuda") and not torch.cuda.is_available():
        raise ModelException("CUDA requested but not available.")
    return torch.device(normalized)


class _autocast_if_available:
    def __init__(self, device: torch.device, dtype: torch.dtype) -> None:
        self._device = device
        self._dtype = dtype
        self._ctx = None

    def __enter__(self) -> None:
        if self._device.type == "cuda" and self._dtype in {torch.float16, torch.bfloat16}:
            self._ctx = torch.autocast(device_type="cuda", dtype=self._dtype)
            self._ctx.__enter__()

    def __exit__(self, exc_type: object, exc: object, tb: object) -> None:
        if self._ctx is not None:
            self._ctx.__exit__(exc_type, exc, tb)
